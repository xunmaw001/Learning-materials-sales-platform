<template>
    ...
    <view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        ...
        <view style="display: flex;flex-direction: column;">
          <view :style='categoryName===item.ziliaoleixing?sactiveItem:sitem' class="tab" v-for="(item,index) in categoryList" :key="index" :class="categoryName===item.ziliaoleixing?'tabActive':''" @tap="categoryClick(item.ziliaoleixing)">{{item.ziliaoleixing}}</view>
        </view>
      </scroll-view>
        ...
        <view v-if="index%6==0" @tap="onDetailTap(product)" class="list-item" :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0 0 40rpx 0","borderColor":"red","backgroundColor":"rgba(6, 6, 6, 0.27)","borderRadius":"20rpx","borderWidth":"0","width":"100%","borderStyle":"solid","height":"320rpx"}'>
                                                                                                              <image :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0 0 8rpx 0","borderColor":"red","backgroundColor":"rgba(255,0,0,0)","borderRadius":"20rpx","borderWidth":"0","width":"100%","borderStyle":"solid","height":"320rpx"}' mode="aspectFill" :src="product.xiangguantupian?baseUrl+product.xiangguantupian.split(',')[0]:''"></image>
        ...
        <view v-if="index%6==1" @tap="onDetailTap(product)" :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0 2% 20rpx 0","borderColor":"red","backgroundColor":"rgba(255,0,0,0)","borderRadius":"20rpx","borderWidth":"0","width":"48%","borderStyle":"solid","height":"auto"}'>
                                                                                                              <image :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0","borderColor":"red","backgroundColor":"rgba(255,0,0,0)","borderRadius":"20rpx","borderWidth":"0","width":"100%","borderStyle":"solid","height":"200rpx"}' mode="aspectFill" :src="product.xiangguantupian?baseUrl+product.xiangguantupian.split(',')[0]:''"></image>
                                                                                                                                                                          <view style="display: flex;justify-content: space-between;">
            <text v-if="userid && isAuth('xuexiziliao','修改')" class="cuIcon-edit" @click.stop="onUpdateTap(product.id)">修改</text>
            <text v-if="!userid && isAuthFront('xuexiziliao','修改')" class="cuIcon-edit" @click.stop="onUpdateTap(product.id)">修改</text>
            <text v-if="userid && isAuth('xuexiziliao','删除')" class="cuIcon-delete" @click.stop="onDeleteTap(product.id)">删除</text>
...
<script>
	export default {
		...
		async onShow() {
            ...
            let res = {};
            if(this.userid) {
                res = await this.$api.page('ziliaoleixing', {page:1,limit:100});
            } else {
                res = await this.$api.list('ziliaoleixing', {page:1,limit:100});
            }
			res.data.list.splice(0,0,{id:0,ziliaoleixing:'全部'})
		...
		methods: {
			...
			async upCallback(mescroll) {
				...
				if(this.categoryName!='全部'){
					params.ziliaoleixing = '%' + this.categoryName + '%'
				}
				...
				if(indexQueryCondition) {
					params['ziliaomingcheng'] = '%' + indexQueryCondition + '%';
