...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','ziliaoleixing');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','ziliaoleixing');
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('ziliaoleixing', this.id);
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discussziliaoleixing/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
			...
			async onFinishTap() {
				...
				await this.$api.update('ziliaoleixing', this.detail);
				this.$utils.msg('审核成功');
