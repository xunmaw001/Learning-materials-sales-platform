<template>
    ...
    <view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        ...
        <view v-if="index%6==0" @tap="onDetailTap(product)" class="list-item" :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0 0 40rpx 0","borderColor":"red","backgroundColor":"rgba(6, 6, 6, 0.27)","borderRadius":"20rpx","borderWidth":"0","width":"100%","borderStyle":"solid","height":"320rpx"}'>
            ...
            <text v-if="userid && isAuth('ziliaoleixing','修改')" class="cuIcon-edit" @click.stop="onUpdateTap(product.id)">修改</text>
            <text v-if="!userid && isAuthFront('ziliaoleixing','修改')" class="cuIcon-edit" @click.stop="onUpdateTap(product.id)">修改</text>
            <text v-if="userid && isAuth('ziliaoleixing','删除')" class="cuIcon-delete" @click.stop="onDeleteTap(product.id)">删除</text>
            <text v-if="!userid && isAuthFront('ziliaoleixing','删除')" class="cuIcon-delete" @click.stop="onDeleteTap(product.id)">删除</text>
          </view>
        </view>
	...
	export default {
		...
		methods: {
			...
			async upCallback(mescroll) {
                ...
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`ziliaoleixing`, params);
                } else {
                    res = await this.$api.list(`ziliaoleixing`, params);
                }
				// 如果是第一页数据置空
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('ziliaoleixing', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search(){
                ...
                let res = {};
                if(this.userid) {
                    res = await this.$api.page(`ziliaoleixing`, searchForm);
                } else {
                    res = await this.$api.list(`ziliaoleixing`, searchForm);
                }
				// 如果是第一页数据置空
